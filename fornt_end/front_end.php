...
class poll_front_end {
	...
	public function save_poll_in_databese() {
		...
		$new_voted_array = $this->sanitize_array($_POST['date_answers']);
		if ($user_information->user_ip) {
			if ($array_of_answered_user)
				foreach ($array_of_answered_user as $answer) {
					$wpdb->query('UPDATE ' . $wpdb->prefix . 'polls SET vote = vote-1 WHERE `question_id` = ' . intval($question_id) . ' AND `answer_name` = ' . intval($answer) . '');
				}
			if ($new_voted_array)
				foreach ($new_voted_array as $new_answer) {
					$wpdb->query('UPDATE ' . $wpdb->prefix . 'polls SET vote = vote+1 WHERE `question_id` = ' . intval($question_id) . ' AND `answer_name` = ' . intval($new_answer) . '');
				}
			$wpdb->update(
	...
	private function genrerte_styles_for_curent_poll($prefix = 0, $poll_design = '') {
		...
		$curent_poll_style = '<style>
			...
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer{
				background-color: ' . $this->hex2rgba('#ffffff', '100') . ';
				width: ' . esc_html($poll_parametrs['poll_answer_width']) . '%;
				height: ' . esc_html($poll_parametrs['poll_answer_height']) . 'px;
				margin-bottom: ' . '5' . 'px;
				border-width:' . '1' . 'px;
				border-radius:' . '5' . 'px;
				border-style:' . 'outset' . ';
			...
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer:hover{
				background-color: ' . $this->hex2rgba('#ffffff', '100') . ';
				width: ' . esc_html($poll_parametrs['poll_answer_width_hower']) . '%;
				height: ' . esc_html($poll_parametrs['poll_answer_height_hower']) . 'px;
				border-width:' . '1' . 'px;
				border-radius:' . '5' . 'px;
				border-style:' . 'outset' . ';
				border-color:' . '#3b5998' . ';
			}
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer .poll_answer_title{
				font-size: ' . '14' . 'px;
				top: ' . esc_html($poll_parametrs['poll_answer_of_the_top']) . 'px;
				left: ' . '5' . 'px;
			}
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer:hover .poll_answer_title{
				font-size: ' . '14' . 'px;
				top: ' . esc_html($poll_parametrs['poll_answer_of_the_top_hower']) . 'px;
				left: ' . '5' . 'px;
			}
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer .pracents_of_the_poll{
				background-color: ' . '#afafaf' . ';
			}
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer:hover .pracents_of_the_poll{
				background-color: ' . '#afafaf' . ';
			}
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer .poll_answer_title{
				color: ' . '#0c0101' . ';
			...
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer .poll_span_voted_count{
				display:' . ('0' ? 'inline-block' : 'none') . ';
				font-size: ' . '13' . 'px;
				right: ' . '7' . 'px;
				top: ' . esc_html($poll_parametrs['poll_count_of_the_top']) . 'px;
				color: ' . '#878787' . ';
				font-family: ' . 'Arial Narrow,Arial,Helvetica Neue,Helvetica,sans-serif' . ';
				font-weight: ' . ('on' ? 'bold' : 'normal') . ';
				font-style: ' . ('on' ? 'italic' : 'normal') . ';
			}
			.poll_main_div_' . esc_html($prefix) . ' .main_div_of_the_poll_answer:hover .poll_span_voted_count{
				display:' . ('1' ? 'inline-block' : 'none') . ';
				font-size: ' . '13' . 'px;
				right: ' . '7' . 'px;
				top: ' . esc_html($poll_parametrs['poll_count_of_the_top_hower']) . 'px;
				color: ' . '#878787' . ';
				font-family: ' . 'Arial Narrow,Arial,Helvetica Neue,Helvetica,sans-serif' . ';
				font-weight: ' . ('on' ? 'bold' : 'normal') . ';
				font-style: ' . ('on' ? 'italic' : 'normal') . ';
			}
			.poll_main_div_' . esc_html($prefix) . ' .radio_or_select_span{
	...
	private function generete_html_front_end_poll($poll_answer_id) {
		...
		$poll_question_and_answers .= '<div class="poll_question">' . wp_filter_post_kses( $question->question ) . '</div>';
		foreach ($answers as $answer) {
			...
			$poll_question_and_answers .= '<div class="div_with_ceckbox_or_select poll_element_' . esc_html($answer->answer_name) . '">';
			$poll_question_and_answers .= '<span class="radio_or_select_span"><input onclick="send_to_databese(' . esc_attr($poll_answer_id) . ',' . esc_attr(self::$prefix_main_poll) . ')" class="poll_inputs" type="' . esc_attr($answer_type) . '" name="poll_' . esc_attr(self::$prefix_main_poll) . '" value="' . esc_attr($answer->answer_name) . '" ' . checked(1, $checked, false) . '></span>';
			$poll_question_and_answers .= '<div class="div_only_answer">';
			$poll_question_and_answers .= '<div class="main_div_of_the_poll_answer" onclick="clicked_in_poll_div(this,' . esc_attr( $poll_answer_id ) . ',' . esc_attr(self::$prefix_main_poll) . ')" title="' . esc_attr($answer->answer) . '">';
			$poll_question_and_answers .= '<div class="pracents_of_the_poll colorwell" style="width: ' . wp_filter_post_kses($this->pracents_of_answer($voted_count, $answer->vote)) . '%;"></div>';
			$poll_question_and_answers .= '<div class="polllabel"><span class="poll_answer_title">' . esc_html($answer->answer) . '</span></div>';
			$poll_question_and_answers .= '<div class="poll_div_voted_count"><span class="poll_span_voted_count"> ' . esc_html($answer->vote) . ' Vote</span></div>';
	...
	/*Function for Getting the User IP*/
	...
	public function generete_front_end_single_poll($poll_answer_id, $poll_design_id) {
		...
		$poll_front_end_code .= '<div class="poll_main_div_' . esc_attr(self::$prefix_main_poll) . ' poll_min_div_cur_answer_' . esc_attr($poll_answer_id) . ' wpdevartpoll">' . stripslashes($this->generete_html_front_end_poll($poll_answer_id)) . '</div>';
		return $poll_front_end_code;
	}
