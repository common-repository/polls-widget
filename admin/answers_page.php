...
?>
	...
	?>
		...
		<script language="javascript" type="text/javascript">
			...
			var count_of_elements = <?php echo  esc_html($max_answer_id); ?>;
		...
		<table class="top_description_table">
			<tr>
				<td width="100%">
					<h2 class="poll_title"><?php echo esc_attr($poll_title); ?></h2>
				</td>
		...
		<form action="admin.php?page=Polls<?php if ($id) echo '&id=' . esc_html($id); ?>" method="post" name="adminForm" class="top_description_table" id="adminForm">
			<div class="param_name">
				<br><br>
				<input type="text" class="poll_name" name="poll_name" placeholder="Enter name here" value="<?php echo esc_attr($poll_name); ?>" style="width:818px">
				<br><br>
			</div>
			...
			<div id="answers_td" class="param_answer">
				...
				<?php if ($answers == '') { ?>
					...
					foreach ($answers as $answer) {
					?>
						<div id="answer_<?php echo esc_attr($answer->answer_name); ?>">
							<input type="text" id="answerpoll_no_<?php echo esc_attr($answer->answer_name); ?>" name="answer[<?php echo esc_attr($answer->answer_name); ?>]" value="<?php echo esc_attr($answer->answer); ?>" onchange="add_answer('<?php echo esc_attr($answer->answer_name); ?>');" class="text_input" style="width:200px;"><input class="button-secondary" type="button" value=" Remove " title=" Remove " onclick="remov_answer('<?php echo esc_attr($answer->answer_name); ?>')"><br>
						</div>
					<?php
					}
					?>
					<div id="answer_<?php echo $max_answer_id ?>">
						<input type="text" id="answerpoll_no_<?php echo esc_attr($max_answer_id); ?>" name="answer[<?php echo esc_attr($max_answer_id); ?>]" value="" onchange="add_answer('<?php echo esc_attr($max_answer_id); ?>');" class="text_input" style="width:200px;"><input class="button-secondary" type="button" value=" Remove " title=" Remove " onclick="remov_answer('<?php echo esc_attr($max_answer_id); ?>')"><br>
					</div>
				<?php } ?>
	...
	private function save_poll() {
		...
		$save_or_no = $wpdb->insert(
			$wpdb->prefix . 'polls_question',
			array(
				'question' => stripslashes(wp_kses_post($_POST['Question'])),
				'name' => htmlspecialchars(stripslashes(sanitize_text_field($_POST['poll_name']))),
			...
			foreach ($answers as $key => $answer) {
				if ($answer) {
					$save_or_no = $wpdb->insert(
						$wpdb->prefix . 'polls',
						array(
							'question_id' => sanitize_text_field( $id_question ),
							'answer' => sanitize_text_field($answer),
							'answer_name' => sanitize_text_field($key)
						),
						array(
							'%d',
							'%s',
		...
		?><div class="updated">
					...
					if (isset($answers[$answer_name]) && isset($asnwer_base_names[$answer_name])) {
						if ($answers[$answer_name]) {
							$wpdb->update(
								...
								array(
									'question_id' => sanitize_text_field($id_question),
									'answer' => sanitize_text_field( $answers[$answer_name] ),
									'answer_name' => sanitize_text_field($answer_name)
								),
								array('id' => sanitize_text_field($asnwer_base_names[$answer_name])),
								array(
									'%d',
									'%s',
									'%d'
							...
							$query = 'DELETE FROM ' . $wpdb->prefix . 'polls WHERE id=' . intval($asnwer_base_names[$answer_name]) . '';
							$wpdb->query($query);
						}
					}
					if (isset($answers[$answer_name]) && !isset($asnwer_base_names[$answer_name])) {
			...
			private function duplicate($id) {
				...
				$save_or_no = $wpdb->insert(
					$wpdb->prefix . 'polls_question',
					array(
						'question' => wp_kses_post( $orginal_poll_question->question ),
						'name' => sanitize_text_field( $orginal_poll_question->name ) . '_copy',
						'answer_type' => sanitize_text_field($orginal_poll_question->answer_type),
						...
						if ($answer) {
							$save_or_no = $wpdb->insert(
								...
								array(
									...
									'answer_name' => sanitize_text_field($answer->answer_name)
								),
								array(
									'%d',
									'%s',
				...
				if ($save_or_no) {
				?><div class="updated">
				<p><strong>Item Duplicated</strong></p>
			</div><?php
				} else {
					?><div id="message" class="error">
				<p>Item Not Duplicated</p>
			</div> <?php
				}
			}
			private function remove_poll($id) {
				...
				$wpdb->query($query);
					?><div class="updated">
			<p><strong>Item Deleted</strong></p>
		</div><?php
			}
